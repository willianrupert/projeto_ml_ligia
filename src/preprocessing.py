...
def feature_engineering(df: pd.DataFrame, scaler=None, is_train: bool = True):
    ...
    is_train=True: Treina o Scaler e transforma os dados (usado no Treino).
    is_train=False: Apenas aplica o Scaler treinado aos novos dados (usado no Teste e Validação).
    """
    df_copy = df.copy()
    
    # 1. Tratamento do Tempo (Matemática Cíclica)
    ...
    # 2. Tratamento do Valor Monetário (Evitando Data Leakage)
    if 'Amount' in df_copy.columns:
        if is_train:
            ...
            if scaler is None:
                raise ValueError("ERRO: Um scaler treinado deve ser passado para dados de teste!")
            # Apenas transforma, baseado nas regras aprendidas no treino
    ...
    # Retorna o dataframe e o molde do scaler para uso futuro
    if is_train:
